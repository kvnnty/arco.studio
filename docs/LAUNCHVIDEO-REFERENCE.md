# LaunchVideo.app — reference & combined Arco MVP

**Source:** Live walkthrough of [launchvideo.app](https://launchvideo.app) (June 2025).

**Purpose:** Document LaunchVideo’s full flow and user journey, then define how **Arco combines** LaunchVideo’s AI-first creation UX with Arco’s **real recording + motion editor** differentiator.

---

## What LaunchVideo is

> “Turn any website into studio-quality motion video.”

**Input:** Product URL **or** text prompt (Pro).  
**Output:** ~30–76s launch video with AI voiceover, generated/animated scenes, brand styling, MP4 export.  
...
| Features | Paste · Sound · Read site · Preview · Share |
...
| Field | Detail |
|-------|--------|
| Describe your video | 50–5000 chars — “Describe your idea, LaunchVideo will bring it to life…” |
...
| Watermark | Toggle launchvideo.app badge (Pro removes) |
...
| Dimension | LaunchVideo | Arco (today) | Arco advantage |
|-----------|-------------|--------------|----------------|
...
| **Voice** | Full TTS voiceover | None (MVP) | LaunchVideo wins on “talking launch video” |
...
> LaunchVideo makes videos *about* your product from a URL.  
> **Arco makes videos *of* your product** — real recordings — with LaunchVideo-grade AI for brief, brand, copy, and scene planning.

---

## Combined Arco MVP — unified flow

Merge LaunchVideo’s **creation simplicity + AI pipeline UX** with Arco’s **recording-first truth**.

### Unified user journey

```mermaid
flowchart TD
  subgraph create [Create — LaunchVideo-style]
  ...
  subgraph edit [Edit — Arco + LaunchVideo]
    J --> K{Refine}
...
### Step 0 — Entry (adopt from LaunchVideo)
...
| LaunchVideo step | Arco adaptation |
|------------------|-----------------|
...
| *(missing in LV)* | **Analyze recording** — duration, optional frame labels |
| Output | `ProjectBrief` + brand kit + draft `Marker[]` |

**UI:** Left chat log (copy LaunchVideo) + right stepper: **Analyze → Draft → Motion → Preview → Export**

Skip LaunchVideo’s Voice/Layout/Scenes AI-gen steps for MVP; replace with **Motion** (apply presets to real recording).

### Step 2 — Draft (combine)

LaunchVideo drafts 11 scenes with visual prompt + VO. Arco drafts **markers** on the recording timeline:

| LaunchVideo scene field | Arco `Marker` field |
|-------------------------|---------------------|
...
### Step 3 — Preview (adopt from LaunchVideo)

| Feature | Arco MVP |
|---------|----------|
| Inline Remotion player | ✅ Already built |
| Scene strip / timeline | Add thumbnail strip like LaunchVideo |
...
### Step 5 — Export (adopt from LaunchVideo)
...
### Step 6 — Projects (adopt from LaunchVideo)
...
### Monetization (LaunchVideo model — phase 2)
...
- [ ] AI-generated scene footage (LaunchVideo’s Scenes step)
...
1. **Dashboard create screen** — LaunchVideo layout + Arco upload
...
8. Voice/TTS (optional parity with LaunchVideo)
