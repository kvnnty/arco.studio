...
function CellValue({ value }: { value: string | boolean }) {
  if (typeof value === "boolean") {
    return value ? (
      ...
      <Minus className="mx-auto size-4 text-[var(--marketing-subtle)]" />
    );
  }
  return <span className="text-[14px] text-[var(--marketing-muted)]">{value}</span>;
}

export function FeatureComparisonTable() {
  return (
    <div className="overflow-x-auto rounded-2xl border border-[var(--marketing-border)]">
      <table className="w-full min-w-[640px] border-collapse text-left">
        <thead>
          <tr className="border-b border-[var(--marketing-border)] bg-[var(--marketing-surface)]">
            <th className="px-6 py-4 text-[13px] font-medium text-[var(--marketing-muted)]">
              Feature
            </th>
          ...
          {featureComparison.categories.map((category) => (
            <Fragment key={category.name}>
              ...
              {category.features.map((feature) => (
                <tr
                  key={feature.name}
                  className="border-b border-[var(--marketing-border)] last:border-0"
                >
